...
export default class AnimationUtils {
  constructor(virtualTime: WaveSurfer = new WaveSurfer({ container: document.createElement('div') })) {
    this.initAnimations(virtualTime);
  }

  /** 显示的动画列表 */
  animationList: Animation[] = [];
  animationVirtualTime: WaveSurfer | undefined = undefined;
  animationTimeoutAbortController: AbortController | undefined = undefined;
  initAnimations = (virtualTime: WaveSurfer) => {
    this.animationVirtualTime = virtualTime;
    this.animationTimeoutAbortController = new AbortController();
    // 拖动进度条後，清除所有动画
    if (this.animationVirtualTime) {
      this.animationVirtualTime.on('seeking', (currentTime: number) => {
        // 清除已进入列表的动画
  ...
  drawAnimations = () => {
    if (this.animationList.length > 0) {
      for (let i = 0; i < this.animationList.length; i++) {
        ...
        // 更新时刻
        currentAnimation.currrentTick = (this.animationVirtualTime?.getCurrentTime() ?? 0) * 1000 - currentAnimation.startTime;
  ...
  animation = (key: string | null, length: number, draw: (t: number) => void, wait: number = 0, onFinish?: (params?: any | null) => void) => {
    const fun = () => {
      ...
      if (existing) {
        ...
        this.animationList.push({ key, length, draw, currrentTick: 0, startTime: (this.animationVirtualTime?.getCurrentTime() ?? 0) * 1000, onFinish });
      }
    };
